class Movie
  ...
  def initialize
    ...
    @movie_api = Guidebox # TODO CHANGE TO MOVIEDB
    @url_api = Guidebox
  end
  ...
  def populate_db_titles
    while !@total_num_movies || @num_retrieved < @total_num_movies
      response = @movie_api.pull_movies({ offset: @num_retrieved, limit: LIMIT_MAX })
      save_movies(response)
      sleep((60 / @movie_api.MAX_PER_MIN) + 0.1)
    end
  end
  ...
  def populate_watchables_data(watchables)
    watchables.each do |movie|
      populate_watchable_data(movie.moviedb_id)
      sleep((60 / @url_api.MAX_PER_MIN) + 0.1)
    end
  end

  def populate_watchable_data(moviedb_id)
    options = { type: "movie", field: 'id', id_type: 'themoviedb', query: id }

    response = @url_api.search_for_movie(options)
    movie = @url_api.pull_movie_data(response["id"])

    watch = Watchable.find_by(moviedb_id: moviedb_id, moviedb_type: "movie")
